...
func NewUrl(ctx *fiber.Ctx) error {
	...
	return ctx.JSON(fiber.Map{
		...
		"url":     fmt.Sprintf("http://localhost:9900/url-shortend/%s", code),
		"status":  201,
	})
}
...
func GetUrlByCode(ctx *fiber.Ctx) error {
	...
	result, err := db.Query(`Select name,description,url,code,visitor,created_at,updated_at from link where code=$1`, code)
	...
	result.Scan(&urlShortend.Name, &urlShortend.Description, &urlShortend.Url, &urlShortend.Code, &urlShortend.Visitor, &urlShortend.CreatedAt, &urlShortend.UpdatedAt)

	return ctx.JSON(urlShortend)
}
